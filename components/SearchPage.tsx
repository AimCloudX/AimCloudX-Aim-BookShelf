...
const SearchPage = () => {
  ...
  const saveComment = async () => {
    ...
    const book: Book = {
      ...
      isbn10: selectedBook.volumeInfo.industryIdentifiers?.find(
        (id: any) => id.type === 'ISBN_10'
      )?.identifier || '',
      isbn13: selectedBook.volumeInfo.industryIdentifiers?.find(
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded shadow-lg w-96">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={saveComment} className="bg-blue-500 text-white px-4 py-2 rounded">
                保存
              </button>
            </div>
          </div>
     ...
     {showInstanceModal && selectedBook && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded shadow-lg w-96">
            ...
            />
            <div className="flex justify-end gap-2">
              <button onClick={closeInstanceModal} className="bg-gray-500 text-white px-4 py-2 rounded">
                キャンセル
              </button>
              <button onClick={saveInstance} className="bg-blue-500 text-white px-4 py-2 rounded">
                保存
              </button>
            </div>
          </div>
